...
/*   Updated: 2026/04/17 12:43:39 by Oery             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_cmd_node	*parse_node_content(t_parser *p, t_cmd_node *node)
{
	...
	// {
	// 	node->data = parser_parse_group(p);
	// 	return (node->data);
	// }
	if (token->type == DOLLAR || token->type == STAR || token->type == STRING)
	{
		node->data = parser_parse_cmd(p);
		return (node->data);
	}
	return (NULL);
}
